...
-- DATA SOTRED AT: ThunxUIDataPerChar.cabsecondary = {{id, tpye}, ...} AND ThunxUIDataPerChar.cabprimary = {{id, tpye}, ...}
local BUTTON_SIZE = E.db.actionbar.bar1.buttonsize + 80 
...
local function ActiveTable()
	if GetActiveTalentGroup() == 1 then
		return _G["ThunxUIDataPerChar"].primary
	else
		return _G["ThunxUIDataPerChar"].secondary
	end
end
...
-- Parent BG frame
cabframe = CreateFrame("Frame", "ThunxUICustomactionbar", ElvUF_Player)
cabframe:SetTemplate("Default")
cabframe:Size(1, BUTTON_SIZE+4)
cabframe:Point("BOTTOMLEFT", ElvUF_Player, "TOPLEFT", 0, 1)
cabframe:SetAlpha(0)
...
local function ManageDividers()
	if #dividers < #b then
		...
		if #dividers == 1 then
			dividers[#dividers]:SetTemplate("Default")
			dividers[#dividers]:Size(1, BUTTON_SIZE+4)
			dividers[#dividers]:Point("BOTTOMLEFT", cabframe, "BOTTOMLEFT", BUTTON_SIZE+3, 0)
		else
			dividers[#dividers]:SetTemplate("Default")
			dividers[#dividers]:Size(1, BUTTON_SIZE+4)
			dividers[#dividers]:Point( "BOTTOMLEFT", dividers[#dividers-1], "BOTTOMLEFT", BUTTON_SIZE+3, 0)
		end
...
f:SetScript("OnEvent", function(self, event)
	if event == "ADDON_LOADED" then
		if not ThunxUIDataPerChar then ThunxUIDataPerChar = {} end
		if not ThunxUIDataPerChar.primary then ThunxUIDataPerChar.primary = {} end
		if not ThunxUIDataPerChar.secondary then ThunxUIDataPerChar.secondary = {} end
	elseif event == "PLAYER_ENTERING_WORLD" then
		emptybutton = MakeDropButton()
		InitButtons()
	elseif event == "ACTIVE_TALENT_GROUP_CHANGED" then
		E.KillTableofFrames(dividers)
		E.Delaythunx(.1, InitButtons)
	end
end)
...
SlashCmdList.CLEARCAB = function()
	_G["ThunxUIDataPerChar"].primary = nil
	_G["ThunxUIDataPerChar"].secondary = nil
	_G["ThunxUIDataPerChar"].cabprimary = nil
	_G["ThunxUIDataPerChar"].cabsecondary = nil
	ReloadUI()
end