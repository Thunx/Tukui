...
function openMail(index)
	if not InboxFrame:IsVisible() then return stopOpening("|cffffff00".."Mail needed") end
	if index == 0 then MiniMapMailFrame:Hide() stopOpening("|cffffff00".."Mail complete") return end
	...
	if (numItems and numItems > 1) or (items > 1 and index <= items) then
		...
		stopOpening("|cffffff00".."Mail complete")
...
function onEvent(frame, event, arg1, arg2, arg3, arg4)
	if event == "UI_ERROR_MESSAGE" then
		if arg1 == ERR_INV_FULL then
			stopOpening("|cffffff00".."Mail stopped")
		end
	end
end
...
button:SetScript("OnEnter", function()
	...
	GameTooltip:AddLine(string.format("%d ".."mail messeage", GetInboxNumItems()), 1, 1, 1)
	GameTooltip:Show()
end)
